...
| 1.   | Volume High Level Biometric Services<br><br>1                      |  |
|------|--------------------------------------------------------------------|--|
| 2.   | Architecture for Biometric Applications<br><br>2                   |  |
| 2.1. | Client-Server Architecture<br><br>2                                |  |
| 3.   | Document Overview<br><br>4                                         |  |
| 3.1. | Terminology<br><br>4                                               |  |
| 3.2. | Naming Conventions<br><br>4                                        |  |
| 3.3. | Namespaces<br><br>5                                                |  |
| 3.4. | XML Schema and Web Service Definition<br><br>5                     |  |
| 3.5. | Interoperability<br><br>5                                          |  |
| 4.   | Interface Overview<br><br>6                                        |  |
| 4.1. | High-Level Biometric Services<br><br>6                             |  |
| 4.2. | Error Handling<br><br>6                                            |  |
| 5.   | High Level Biometric Services API<br><br>7                         |  |
| 5.1. | Namespaces<br><br>7                                                |  |
| 5.2. | Data Types<br><br>7                                                |  |
| 5.3. | Fault Types<br><br>23                                              |  |
| 5.4. | Operations<br><br>27                                               |  |
| 5.5. | Service-Device Description Schema<br><br>36                        |  |
| 6.   | Example (Non-Normative)<br><br>62                                  |  |
| 6.1. | Service-Device Description<br><br>62                               |  |
| 7.   | Client-Server Connection Scenarios<br><br>64                       |  |
| 7.1. | Connection via TCP/IP<br><br>64                                    |  |
| 7.2. | Connection via USB<br><br>65                                       |  |
| 8.   | Service Definitions<br><br>67                                      |  |
| 8.1. | Service Definition Facial Image Acquisition System<br><br>67       |  |
| 8.2. | Service Definition Basic Facial Image Acquisition System<br><br>74 |  |
...
# <span id="page-7-0"></span>**3. Document Overview**
...
The definitions of the [HLBS](#page-99-1) [API](#page-99-6) are provided in HLBS1v1.wsdl. The schema for the service-device description is provided in hlbs\_service1v1.xsd.
...
Represents the impression type as specified in [\[BIB\\_ISO\\_FINGER\]](#page-100-2) (e.g. finger or palm). Derived from xsd:un signedInt.
...
```
#### <span id="page-19-1"></span>**5.2.12. ServiceInformation**
...
A boolean device property value. Derived from [hlbs:type.device.properties.base](#page-43-1) .
...
A string device property value. Derived from [hlbs:type.device.properties.base](#page-43-1) .
...
An integer configuration value. Derived from [hlbs:type.configuration.base](#page-46-0) .
...
An impression type as defined in [\[BIB\\_ISO\\_FINGER\]](#page-100-2) (e.g. finger or palm). Derived from xs:unsignedInt.
...
A iris code as defined in [\[BIB\\_ISO\\_IRIS\].](#page-100-4) Derived from xs:unsignedInt.
...
A biometric code configuration value. Derived from [hlbs:type.configuration.base](#page-46-0) .
...
A data format configuration value. Derived from [hlbs:type.configuration.base](#page-46-0) .
...
An image configuration value. Derived from [hlbs:type.configuration.base](#page-46-0) .
...
A biometric code list feedback element. Derived from [hlbs:type.feedback.base](#page-58-1) .
...
An icon feedback element. Derived from [hlbs:type.feedback.base](#page-58-1) .
...
A text feedback element. Derived from [hlbs:type.feedback.base](#page-58-1) .
...
<span id="page-68-1"></span>![](_page_68_Figure_1.jpeg)

**Figure 7.2.** Architecture via TCP/IP

# <span id="page-68-0"></span>**7.2. Connection via USB**

In the [Universal Serial Bus \(USB\)](#page-99-13) connection scenario the system that is using [HLBS is split into two compo](#page-99-1) nents. The first component is the actual unit which processes the request (e.g. acquisition of a facial image). This unit is connected via an [USB](#page-99-13)-cable to the client computer where the second component of the system resides. The component on the client computer acts as a driver and implements the [HLBS.](#page-99-1) This architecture is also shown in [Figure 7.3](#page-69-0). The component on the client computer SHALL have support for configuring the port on which the [HLBS](#page-99-1) runs without the need to call [configureService](#page-33-0). The set port SHALL only be available as loopback interface.

<span id="page-69-0"></span>![](_page_69_Figure_1.jpeg)

**Figure 7.3.** Architecture via USB

# <span id="page-70-0"></span>**8. Service Definitions**

Due to the generic structure of [HLBS](#page-99-1) (e.g. support for custom user commands implemented by developers) it is necessary to define Service Definitions. These Service Definitions show different characteristics for each individual service including their minimum requirements.

# <span id="page-70-1"></span>**8.1. Service Definition Facial Image Acquisition System**

This Service Definition specifies requirements for a [Facial Image Acqisition System \(FIAS\)](#page-99-14) that implements [HLBS](#page-99-1) as communication interface.

There are two operation modes that SHALL be supported by the [FIAS.](#page-99-14) These modes are the automated mode and the manual mode. Both modes SHALL be implemented within the following service definition.

# **8.1.1. ServiceInformation**

When the [getAllServices](#page-30-1) operation is requested at least the [ServiceInformation](#page-19-1) shown in [Table 8.1](#page-70-2) SHALL be returned. Further parameters are vendor specific and SHALL be set as well.
...
| Version      | Version of BSI TR-03121 of<br>the implemented service. | xsd:<br>string | e.g. 5.3                                |

**Table 8.1** FIAS ServiceInformation

# **8.1.2. Configuration**

At least the configuration options listed in [Table 8.2](#page-70-3) SHALL be available for the [configureService](#page-33-0) ope ration. These configuration options including their allowed and default values SHALL also be part of the ser viceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.
...
When the [signalUserCommand](#page-35-1) operation is executed the user commands shown in [Table 8.3](#page-71-0) SHALL be supported in case the existence column is set to REQUIRED and MAY be co lumn is set to OPTIONAL. Conditional commands SHALL only be available if the manual operation mode is in use, except for the CropManually, acceptImage and rejectImage command. In case a user command is not al lowed at a certain point of time during the execution of the service the user SHALL be informed via the [ge](#page-35-0) [tServiceFeedback](#page-35-0) operation. The user commands of [Table 8.3](#page-71-0) SHALL also be present within the serviceDe scriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.
...
| Capture      | M/C    | Force capturing the current<br>ly showing camera's field of<br>view. This overrules the result<br>of a live-QA. This command<br>SHALL only usable when the<br>live image is visible during<br>the capture process. | -             | -               |
...
<span id="page-71-1"></span><sup>1</sup> Mandatory / Optional / Conditional
...
| Increment<br>Vertical<br>Position   | C      | Stepwise increment the ver<br>tical position of the came<br>ra's field of view by the defi<br>ned value in cm, maximum to<br>the highest position. In case a<br>multi camera solution is used,<br>it SHALL NOT be possible to<br>set the camera's field of view<br>to a position where a merge<br>image of two or more came<br>ras is created. This command<br>SHALL only be available in<br>manual mode. | Step<br>xsd:int       | Arbitrary Value                                                                      |

<sup>1</sup> Mandatory / Optional / Conditional

| Parameter ID                         | M/O/C1 | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Name and type                      | Possible Values                                                                                                                                                                                                                                                |
|--------------------------------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Decrement<br>Vertical<br>Position    | C      | Stepwise decrement the ver<br>tical position of the came<br>ra's field of view by the defi<br>ned value in cm, maximum to<br>the lowest position. In case a<br>multi camera solution is used,<br>it SHALL NOT be possible to<br>set the camera's field of view<br>to a position where a merge<br>image of two or more came<br>ras is created. This command<br>SHALL only be available in<br>manual mode.                                                                                                        | Step<br>xsd:int                    | Arbitrary Value                                                                                                                                                                                                                                                |
| SetAbsolute<br>Illumination<br>Level | C      | Set the absolute face illumi<br>nation-brightness. This com<br>mand SHALL only be avail<br>able in manual mode.                                                                                                                                                                                                                                                                                                                                                                                                 | Level<br>xsd:float                 | Range: [0, , 1]<br>Minimum brightness: 0<br>Middle brightness: 0.5<br>Maximum brightness:<br>1                                                                                                                                                                 |
...
When the [getServiceFeedback](#page-35-0) operation is executed the feedbackElements shown in [Table 8.4](#page-75-0) SHALL be returned in case the existence column is set to REQUIRED and MAY be returned in case the existence column is set to OPTIONAL. Conditional feedback SHALL only be available if the manual operation mode is in use. Furthermore the userCommands\* SHALL contain the [hlbs:UserCommandInfo](#page-21-2) for each implemented [hlbs:U](#page-25-0) [serCommand](#page-25-0) that is currently allowed to be used. E.g. for the [hlbs:UserCommand](#page-25-0) IncrementIlluminationLevel the value not-allowed is to be returned if the maximum illumination-brightness already reached. The pos sible feedbackElements\* of [Table 8.4](#page-75-0) SHALL also be part of the serviceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
<span id="page-75-1"></span><sup>2</sup> Mandatory / Optional / Conditional
...
| QAFeedback                       | M/C    | Returns the Face<br>Feedback node of the<br>TR-03121 XML Sche<br>ma (see hlbs4v7.xsd<br>for volumes BCL and<br>IMA or hlbs5v1.xsd<br>for volumes GID and<br>ARE) containing all<br>relevant quality infor<br>mation to assist the<br>operator in his or her<br>quality assessment.<br>This feedback SHALL<br>only be returned for<br>the manual operator<br>assessment after the<br>FIAS has made a cap<br>ture. | xsd:<br>string | See TR-03121 XML Schema                                               |
...
The [getResults](#page-37-0) operation returns the resultElements\* as [hlbs:KeyValue](#page-24-0) . The key-value pairs that SHALL be returned are shown in [Table 8.5.](#page-77-1) The possible results of [Table 8.4](#page-75-0) SHALL also be part of the serviceDe scriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
| FaceAcquisition | Contains the full XML root element<br>FaceAcquisition (see TR-03121<br>XML, biotypes4v7.xsd for volumes<br>BCL and IMA or biotypes5v1.xsd<br>for volumes GID and ARE) which logs<br>the entire acquisition process, inclu<br>ding quality information and the fi<br>nally accepted facial image as record.<br>The data format, compression and file<br>size of the record SHALL comply with<br>the configured ApplicationProfile. | xsd:<br>string | See TR-03121 XML Schema                                                                                                                                                                                                                                              |

**Table 8.5** FIAS Result Elements

# <span id="page-77-0"></span>**8.2. Service Definition Basic Facial Image Acquisition System**

This Service Definition specifies requirements for a Facial Image Acquisition System with basic functionality (e.g. used for Supervised Facial Image Acquisition without [Central Identity Register \(CIR\)](#page-99-15) Lookup) that imple ments [HLBS](#page-99-1) as communication interface.
...
When the [getAllServices](#page-30-1) operation is requested at least the [ServiceInformation](#page-19-1) shown in [Table 8.6](#page-78-0) SHALL be returned. Further parameters are vendor specific and SHALL be set as well.
...
At least the configuration options listed in [Table 8.7](#page-78-1) SHALL be available for the [configureService](#page-33-0) ope ration. These configuration options including their allowed and default values SHALL also be part of the ser viceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.
...
When the [signalUserCommand](#page-35-1) operation is executed the user commands shown in [Table 8.8](#page-78-2) SHALL be supported in case the existence column is set to REQUIRED and MAY be co lumn is set to OPTIONAL. Conditional commands SHALL only be available if the manual operation mode is in use, except for the CropManually, acceptImage and rejectImage command. In case a user command is not al lowed at a certain point of time during the execution of the service the user SHALL be informed via the [ge](#page-35-0) [tServiceFeedback](#page-35-0) operation. The user commands of [Table 8.8](#page-78-2) SHALL also be present within the serviceDe scriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.
...
<span id="page-78-3"></span><sup>3</sup> Mandatory / Optional / Conditional
...
When the [getServiceFeedback](#page-35-0) operation is executed the feedbackElements shown in [Table 8.9](#page-80-0) SHALL be returned in case the existence column is set to REQUIRED and MAY be returned in case the existence column is set to OPTIONAL. Conditional feedback SHALL only be available if the manual operation mode is in use. Furthermore the userCommands\* SHALL contain the [hlbs:UserCommandInfo](#page-21-2) for each implemented [hlbs:U](#page-25-0) [serCommand](#page-25-0) that is currently allowed to be used. E.g. for the [hlbs:UserCommand](#page-25-0) IncrementIlluminationLevel the value not-allowed is to be returned if the maximum illumination-brightness already reached. The pos sible feedbackElements\* of [Table 8.9](#page-80-0) SHALL also be part of the serviceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
<span id="page-80-1"></span><sup>4</sup> Mandatory / Optional / Conditional
...
| QAFeedback                 | M/C    | Returns the Face<br>Feedback node of the<br>TR-03121 XML Sche<br>ma (see hlbs4v7.xsd<br>for volumes BCL and<br>IMA or hlbs5v1.xsd<br>for volumes GID and<br>ARE) containing all<br>relevant quality infor<br>mation to assist the<br>operator in his or her<br>quality assessment.<br>This feedback SHALL<br>only be returned for<br>the manual opera<br>tor assessment after<br>the Basic Facial Image<br>Acquisition System<br>has made a capture. | xsd:<br>string | See TR-03121 XML Schema                                               |
...
The [getResults](#page-37-0) operation returns the resultElements\* as [hlbs:KeyValue](#page-24-0) . The key-value pairs that SHALL be returned are shown in [Table 8.10](#page-82-0). The possible results of [Table 8.9](#page-80-0) SHALL also be part of the serviceDe scriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
# <span id="page-83-0"></span>**8.3. Service Definition Fingerprint Acquisition**

This service definition specifies requirements for a system acquiring fingerprints.

# **8.3.1. ServiceInformation**

When the [getAllServices](#page-30-1) operation is requested at least the [hlbs:ServiceInformation](#page-19-1) shown in [Ta](#page-83-1) [ble 8.11](#page-83-1) SHALL be returned. Further parameters are vendor specific and SHALL be set as well.
...
**Table 8.11** Fingerprint Acquisition ServiceInformation

# **8.3.2. Configuration**

At least the configuration options listed in [Table 8.12](#page-83-2) SHALL be available for the [configureService](#page-33-0) ope ration. These configuration options including their allowed and default values SHALL also be part of the ser viceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.
...
**Table 8.12** Fingerprint Acquisition Configuration

#### **8.3.3. User Commands**

When the [signalUserCommand](#page-35-1) operation is executed the user commands shown in [Table 8.13](#page-84-0) SHALL be supported by the service. In case a user command is not allowed at a certain point of time during the execution of the service the user SHALL be informed via the [getServiceFeedback](#page-35-0) operation. The user commands of [Table 8.13](#page-84-0) SHALL also be present within the serviceDescriptionXML that is returned with the [getService](#page-31-0) [Description](#page-31-0) operation.
...
**Table 8.13** Fingerprint Acquisition UserCommands

#### **8.3.4. Feedback**

Within the [hlbs:Feedback](#page-26-1) the feedbackElements\* of [Table 8.14](#page-86-0) as [hlbs:KeyValue](#page-24-0) SHALL be returned. Furthermore the userCommands\* SHALL contain the [hlbs:UserCommandInfo](#page-21-2) for each implemented [hlbs:U](#page-25-0) [serCommand](#page-25-0) . The possible feedbackElements\* of [Table 8.14](#page-86-0) SHALL also be part of the serviceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) .

<span id="page-86-0"></span>

| Parameter ID                             | M/O/C5 | Description                                                                                                                                                   | Type                           | Possible Values                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
|------------------------------------------|--------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| LiveStatus                               | O      | Transmission of sta<br>tus information du<br>ring the continuous<br>capture process (live<br>view) for further pro<br>cessing within the cli<br>ent software. | xsd:<br>string                 | Initializing<br>(service initializes),<br>SelectMissingFingers<br>(operator is asked to select<br>missing fingers)<br>SearchingFingers<br>(searching for fingers),<br>FingersRecognized<br>(fingers detected),<br>Capturing<br>(capture is running),<br>ReduceFinger<br>Pressure<br>(fingers are placed with too<br>much pressure),<br>RaiseFingerPressure<br>(fingers are placed with too<br>less pressure or are placed<br>partly in the air),<br>MoveFingersLeft<br>(fingers are too far right),<br>MoveFingersRight<br>(fingers are too far left),<br>MoveFingersForward<br>(fingers are too far back),<br>MoveFingersBackward<br>(fingers are too far ahead),<br>KeepFingersStill<br>(fingers are too much in mo<br>vement),<br>PerformingQA<br>(software-based QA is run<br>ning),<br>AssessQuality<br>(operator is asked to accept or<br>reject an acquired image) |
...
| Intermediate<br>FingerAmount<br>Mismatch | O      | There is a problem<br>with the amount of<br>fingers that have been<br>captured with the last<br>intermediate finger<br>print image.                           | xsd:<br>string                 | TooLessFingers<br>Captured<br>(less fingers than expected<br>have been captured),<br>TooManyFingers<br>Captured<br>(more fingers than expected<br>have been captured)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |

<span id="page-86-1"></span><sup>5</sup> Mandatory / Optional / Conditional
...
| Intermediate<br>FingerFeedback        | M      | Returns the Finger<br>Feedback node of the<br>TR-03121 XML Sche<br>ma (see hlbs4v7.xsd<br>for volumes BCL and<br>IMA or hlbs5v1.xsd<br>for volumes GID and<br>ARE) containing all<br>relevant quality, PAD<br>and uniqueness infor<br>mation to assist the<br>operator in his or her<br>quality assessment.<br>This feedback SHALL<br>only be returned for<br>the manual opera<br>tor assessment after a<br>finger capture has be<br>en made. | xsd:<br>string                 | See TR-03121 XML Schema                                                                                                                                         |

**Table 8.14** Fingerprint Acquisition Feedback Elements

# **8.3.5. Results**

The [getResults](#page-37-0) operation returns the resultElements\* as [hlbs:KeyValue](#page-24-0) . The key-value pairs that SHALL be returned are shown in [Table 8.15.](#page-87-0) The possible results of [Table 8.14](#page-86-0) SHALL also be part of the service DescriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
<sup>5</sup> Mandatory / Optional / Conditional

| Parameter ID      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                | Type           | Possible Values            |
|-------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|----------------------------|
| FingerAcquisition | Contains the full XML root element Finge<br>rAcquisition (see TR-03121 XML, bioty<br>pes4v7.xsd for volumes BCL and IMA or bio<br>types5v1.xsd for volumes GID and ARE),<br>which logs the entire acquisition process, infor<br>mation about quality, PAD and uniqueness as<br>well as the finally accepted fingerprint(s) re<br>cord(s). The data format, compression and file<br>size of the record SHALL comply with con<br>figured ApplicationProfile. | xsd:<br>string | See TR-03121 XML<br>Schema |

**Table 8.15** Fingerprint Acquisition Result Elements

# <span id="page-88-0"></span>**8.4. Service Definition Rolled Fingerprint Acquisition**

This service definition specifies requirements for a system acquiring rolled fingerprints.

### **8.4.1. ServiceInformation**

When the [getAllServices](#page-30-1) operation is requested at least the [hlbs:ServiceInformation](#page-19-1) shown in [Ta](#page-88-1) [ble 8.16](#page-88-1) SHALL be returned. Further parameters are vendor specific and SHALL be set as well.
...
**Table 8.16** Rolled Fingerprint Acquisition ServiceInformation

# **8.4.2. Configuration**

At least the configuration options listed in [Table 8.17](#page-88-2) SHALL be available for the [configureService](#page-33-0) ope ration. These configuration options including their allowed and default values SHALL also be part of the ser viceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.
...
|---------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|--------------------------------------------------------------------------------------------------|
| Purpose                   | The purpose of the fingerprint image<br>acquisition.                                                                                                                 | hlbs:<br>ServiceType               | enrolment,identification,<br>verification                                                        |
| ApplicationProfile        | Relevant Application Profile to be<br>used for the acquisition process and<br>its results.                                                                           | hlbs:<br>Application<br>Profile    | Choice of the implemented Ap<br>plication Profiles (e.g. ARE_<br>ArrivalAttestation<br>Document) |
...
**Table 8.17** Rolled Fingerprint Acquisition Configuration

# **8.4.3. User Commands**

When the [signalUserCommand](#page-35-1) operation is executed the user commands shown in [Table 8.18](#page-89-0) SHALL be supported by the service. In case a user command is not allowed at a certain point of time during the execution of the service the user SHALL be informed via the [getServiceFeedback](#page-35-0) operation. The user commands of [Table 8.13](#page-84-0) SHALL also be present within the serviceDescriptionXML that is returned with the [getService](#page-31-0) [Description](#page-31-0) operation.
...
**Table 8.18** Rolled Fingerprint Acquisition UserCommands

#### **8.4.4. Feedback**

Within the [hlbs:Feedback](#page-26-1) the feedbackElements\* of [Table 8.14](#page-86-0) as [hlbs:KeyValue](#page-24-0) SHALL be returned. Furthermore the userCommands\* SHALL contain the [hlbs:UserCommandInfo](#page-21-2) for each implemented [hlbs:U](#page-25-0)

[serCommand](#page-25-0) . The possible feedbackElements\* of [Table 8.19](#page-90-0) SHALL also be part of the serviceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
<span id="page-90-1"></span><sup>6</sup> Mandatory / Optional / Conditional

| Parameter ID                                        | M/O/C6 | Description                                                                                                                                                                                                                                                                                                                                                                                                                                   | Type           | Possible Values                                                                                                                                                                                                                                                                                                                                                                                                        |
|-----------------------------------------------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Intermediate<br>FingerFeedback                      | M      | Returns the Finger<br>Feedback node of the<br>TR-03121 XML Sche<br>ma (see hlbs4v7.xsd<br>for volumes BCL and<br>IMA or hlbs5v1.xsd<br>for volumes GID and<br>ARE) containing all<br>relevant quality, PAD<br>and uniqueness infor<br>mation to assist the<br>operator in his or her<br>quality assessment.<br>This feedback SHALL<br>only be returned for<br>the manual opera<br>tor assessment after a<br>finger capture has be<br>en made. | xsd:<br>string | See TR-03121 XML Schema                                                                                                                                                                                                                                                                                                                                                                                                |
| Intermediate<br>ControlVerificati<br>on<br>Feedback | O      | Returns the result of<br>the control verifica<br>tion. This feedback<br>SHALL be returned<br>for the manual opera<br>tor assessment.                                                                                                                                                                                                                                                                                                          | xsd:<br>string | undetermined<br>(Application was unable to<br>receive a verification result),<br>successful<br>(Application determined a<br>match between a reference<br>and the captured fingerprint),<br>failed<br>(Application determines a no<br>match between captured fin<br>gerprint and reference fin<br>gerprint, but a match bet<br>ween captured fingerprint<br>and another finger of the set<br>of reference fingerprints) |

**Table 8.19** Rolled Fingerprint Acquisition Feedback Elements

### **8.4.5. Results**

The [getResults](#page-37-0) operation returns the resultElements\* as [hlbs:KeyValue](#page-24-0) . The key-value pairs that SHALL be returned are shown in [Table 8.20.](#page-91-0) The possible results of [Table 8.19](#page-90-0) SHALL also be part of the service DescriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
<sup>6</sup> Mandatory / Optional / Conditional

| Parameter ID      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                | Type           | Possible Values            |
|-------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|----------------------------|
| FingerAcquisition | Contains the full XML root element Finge<br>rAcquisition (see TR-03121 XML, bioty<br>pes4v7.xsd for volumes BCL and IMA or bio<br>types5v1.xsd for volumes GID and ARE),<br>which logs the entire acquisition process, infor<br>mation about quality, PAD and uniqueness as<br>well as the finally accepted fingerprint(s) re<br>cord(s). The data format, compression and file<br>size of the record SHALL comply with con<br>figured ApplicationProfile. | xsd:<br>string | See TR-03121 XML<br>Schema |

**Table 8.20** Rolled Fingerprint Acquisition Result Elements

# <span id="page-92-0"></span>**8.5. Service Definition for Self-Service System**

In this section [HLBS](#page-99-1) service definitions are given that used for external evaluation of biometric acquisiti on components within [self-service systems \(SSSs\)](#page-99-16) individually without executing the entire [SSS](#page-99-16) process. The service definitions stated below mirrors feedback that is shown to the biometric subject infront of the [SSS](#page-99-16) and returns a result that would also be returned in productive mode. Currently, this service definition is only intended for the BCL volume of this technical guideline.

# **8.5.1. Automated Acquisition of Slap Fingerprints**

#### **8.5.1.1. ServiceInformation**

When the [getAllServices](#page-30-1) operation is requested at least the [hlbs:ServiceInformation](#page-19-1) shown in [Ta](#page-92-1) [ble 8.21](#page-92-1) SHALL be returned. Further parameters are vendor specific and SHALL be set as well.

<span id="page-92-1"></span>

| Parameter ID | Description                                            | Type           | Value                                           |
|--------------|--------------------------------------------------------|----------------|-------------------------------------------------|
| Id           | Unique UUID of the service.                            | xsd:<br>string | eb299a2a-00e6-4e3d-a569-d1e4cf<br>d2e8fa        |
| Name         | Name of the service.                                   | xsd:<br>string | Automated Acquisition Slap Fin<br>gerprints SSS |
| Version      | Version of BSI TR-03121 of<br>the implemented service. | xsd:<br>string | e.g. 5.3                                        |

**Table 8.21** Automated Acquisition of Slap Fingerprints ServiceInformation

#### **8.5.1.2. Configuration**

At least the configuration options listed in [Table 8.22](#page-92-2) SHALL be available for the [configureService](#page-33-0) ope ration. These configuration options including their allowed and default values SHALL also be part of the ser viceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.

<span id="page-92-2"></span>

| Parameter ID | M/O7 | Description                                                       | Type                       | Possible Values                                                             |
|--------------|------|-------------------------------------------------------------------|----------------------------|-----------------------------------------------------------------------------|
...
<span id="page-92-3"></span><sup>7</sup> Mandatory / Optional
...
**Table 8.22** Automated Acquisition of Slap Fingerprints Configuration

#### **8.5.1.3. User Commands**

When the [signalUserCommand](#page-35-1) operation is executed the user commands shown in [Table 8.23](#page-93-0) SHALL be supported by the service. In case a user command is not allowed at a certain point of time during the execution of the service the user SHALL be informed via the [getServiceFeedback](#page-35-0) operation. The user commands of [Table 8.23](#page-93-0) SHALL also be present within the serviceDescriptionXML that is returned with the [getService](#page-31-0) [Description](#page-31-0) operation.

<span id="page-93-0"></span>

| Abort/Terminate the running service. Command SHALL be executable at any time.<br>Cancel |  |
|-----------------------------------------------------------------------------------------|--|

**Table 8.23** Automated Acquisition of Slap Fingerprints UserCommands

#### **8.5.1.4. Feedback**

Within the [hlbs:Feedback](#page-26-1) the feedbackElements\* of [Table 8.24](#page-93-1) as [hlbs:KeyValue](#page-24-0) SHALL be returned. Furthermore the userCommands\* SHALL contain the [hlbs:UserCommandInfo](#page-21-2) for each implemented [hlbs:U](#page-25-0) [serCommand](#page-25-0) . The possible feedbackElements\* of [Table 8.24](#page-93-1) SHALL also be part of the serviceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) .

<span id="page-93-1"></span>

| Parameter ID | M/O8 | Description                                                           | Type                            | Possible Values |
|--------------|------|-----------------------------------------------------------------------|---------------------------------|-----------------|
...
<sup>7</sup> Mandatory / Optional

<span id="page-93-2"></span><sup>8</sup> Mandatory / Optional
...
**Table 8.24** Automated Acquisition of Slap Fingerprints Feedback Elements

#### **8.5.1.5. Results**

The [getResults](#page-37-0) operation returns the resultElements\* as [hlbs:KeyValue](#page-24-0) . The key-value pairs that SHALL be returned are shown in [Table 8.25.](#page-95-0) The possible results of [Table 8.24](#page-93-1) SHALL also be part of the service DescriptionXML that is returned with the [getServiceDescription](#page-31-0) .

<sup>8</sup> Mandatory / Optional

<span id="page-95-0"></span>

| Parameter ID      | M/O9 | Description                                                                                                                                                                                                                                                                                                                                                                                        | Type           | Possible Values                                                                                                                                                                                                                                                                                                                                                                          |
|-------------------|------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Result            | M    | Description of the result<br>of the fingerprint image<br>acquisition.                                                                                                                                                                                                                                                                                                                              | xsd:<br>string | Success<br>(acquisition successful),<br>Canceled<br>(acquisition canceled),<br>Timeout<br>(acquisition aborted by<br>system due to timeout),<br>BiometricSubject<br>Disappeared<br>(acquisition aborted by<br>system due to disap<br>pearance of biometric<br>subject),<br>FingerAmount<br>Mismatch<br>(acquisition failed due<br>to finger amount mis<br>match for final slap<br>image) |
| FingerAcquisition | M    | Contains the full XML<br>root element FingerAc<br>quisition (see TR-03121<br>XML, biotypes4v7.xsd<br>for volumes BCL and IMA<br>or biotypes5v1.xsd for<br>volumes GID and ARE)<br>with all relevant informa<br>tion about the process in<br>cluding its results as re<br>cords. The requirements<br>of the SSS Application<br>Profile of the BCL Volu<br>me of this technical gui<br>deline apply. | xsd:<br>string | See TR-03121 XML<br>Schema                                                                                                                                                                                                                                                                                                                                                               |

**Table 8.25** Automated Acquisition of Slap Fingerprints Result Elements

# **8.5.2. Automated Acquisition of Facial Images**

# **8.5.2.1. ServiceInformation**

When the [getAllServices](#page-30-1) operation is requested at least the [hlbs:ServiceInformation](#page-19-1) shown in [Ta](#page-95-2) [ble 8.26](#page-95-2) SHALL be returned. Further parameters are vendor specific and SHALL be set as well.
...
**Table 8.26** Automated Acquisition of Facial Images ServiceInformation

<span id="page-95-1"></span><sup>9</sup> Mandatory / Optional

### **8.5.2.2. Configuration**

At least the configuration options listed in [Table 8.27](#page-96-0) SHALL be available for the [configureService](#page-33-0) ope ration. These configuration options including their allowed and default values SHALL also be part of the ser viceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) operation.

<span id="page-96-0"></span>

| Parameter ID | M/O10 | Description                                                                                                                       | Type                 | Possible Values                 |
|--------------|-------|-----------------------------------------------------------------------------------------------------------------------------------|----------------------|---------------------------------|
| Purpose      | M     | The purpose of the fa<br>cial image acquisition.                                                                                  | hlbs:<br>ServiceType | enrolment, verifi<br>cation     |
| TimeoutMs    | M     | Maximum time in ms<br>after which the acqui<br>sition process will ab<br>ort, in case no facial<br>images have been ac<br>quired. | xsd:int              | Arbitrary value<br>(default: 0) |

**Table 8.27** Automated Acquisition of Facial Images Configuration

#### **8.5.2.3. User Commands**

When the [signalUserCommand](#page-35-1) operation is executed the user commands shown in [Table 8.28](#page-96-2) SHALL be supported by the service. In case a user command is not allowed at a certain point of time during the execution of the service the user SHALL be informed via the [getServiceFeedback](#page-35-0) operation. The user commands of [Table 8.28](#page-96-2) SHALL also be present within the serviceDescriptionXML that is returned with the [getService](#page-31-0) [Description](#page-31-0) operation.
...
**Table 8.28** Automated Acquisition of Facial Images UserCommands

#### **8.5.2.4. Feedback**

<span id="page-96-1"></span>Within the [hlbs:Feedback](#page-26-1) the feedbackElements\* of [Table 8.29](#page-97-0) as [hlbs:KeyValue](#page-24-0) SHALL be returned. Furthermore the userCommands\* SHALL contain the [hlbs:UserCommandInfo](#page-21-2) for each implemented [hlbs:U](#page-25-0) [serCommand](#page-25-0) . The possible feedbackElements\* of [Table 8.29](#page-97-0) SHALL also be part of the serviceDescriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
**Table 8.29** Automated Acquisition of Facial Images Feedback Elements

#### **8.5.2.5. Results**

The [getResults](#page-37-0) operation returns the resultElements\* as [hlbs:KeyValue](#page-24-0) . The key-value pairs that SHALL be returned are shown in [Table 8.30.](#page-98-0) The possible results of [Table 8.29](#page-97-0) SHALL also be part of the service DescriptionXML that is returned with the [getServiceDescription](#page-31-0) .
...
| FaceAcquisition | M     | Contains the full XML<br>root element Face<br>Acquisition (see<br>TR-03121 XML, bio<br>types4v7.xsd for vo<br>lumes BCL and IMA or<br>biotypes5v1.xsd for<br>volumes GID and ARE)<br>with all relevant infor<br>mation about the pro<br>cess including its results<br>as records. The require<br>ments of the SSS Appli<br>cation Profile of the BCL<br>Volume of this technical<br>guideline apply. | xsd:<br>string | See TR-03121 XML Sche<br>ma                                                                                                                                                                                                                                            |

**Table 8.30** Automated Acquisition of Facial Images Result Elements

<span id="page-98-1"></span><sup>12</sup> Mandatory / Optional

# <span id="page-99-0"></span>**List of Abbreviations**

<span id="page-99-16"></span><span id="page-99-15"></span><span id="page-99-14"></span><span id="page-99-13"></span><span id="page-99-12"></span><span id="page-99-11"></span><span id="page-99-10"></span><span id="page-99-9"></span><span id="page-99-8"></span><span id="page-99-7"></span><span id="page-99-6"></span><span id="page-99-5"></span><span id="page-99-4"></span><span id="page-99-3"></span><span id="page-99-2"></span><span id="page-99-1"></span>
...
# <span id="page-100-0"></span> **Bibliography**

- <span id="page-100-3"></span>[BIB\_ISO\_FACE] *ISO/IEC 19794-5:2005 "Information technology - Biometric data interchange formats – Part 5: Face image data".*
- <span id="page-100-2"></span>[BIB\_ISO\_FINGER] *ISO/IEC 19794-4:2005 "Information technology - Biometric data interchange formats – Part 4: Finger image data".*
- <span id="page-100-4"></span>[BIB\_ISO\_IRIS] *ISO/IEC 19794-6:2005 "Information technology - Biometric data interchange formats - Part 6: Iris image data".*
- <span id="page-100-5"></span>[BIB\_ISO\_MINUTIAE] *ISO/IEC 19794-2:2005 "Information technology - Biometric data interchange formats – Part 2: Finger minutiae data".*
- <span id="page-100-1"></span>[BIB\_RFC2119] *RFC 2119: Key words for use in RFCs to Indicate Requirement Levels.*