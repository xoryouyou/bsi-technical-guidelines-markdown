...
![](markdown/tr/BSI-TR-02103/BSI-TR-02103.pdf-0001-00.png)
...
|---|---|---|
||8.4.3|Pfadvalidierung mit erweiterter Funktionalität zur Verarbeitung der Name Constraints|
||Zertifikatserweiterung ...................................................................................................................................................................... 54||
...
||8.5.3|Erweiterung der Revokationsprüfung um die Verarbeitung von Sperrlisten für eine|
||Untermenge der möglichen Sperrgründe ............................................................................................................................... 58||
...
X.509 Zertifikate bilden einen wichtigen Pfeiler der IT-Sicherheit, indem sie die Realisierung von Public-KeyInfrastrukturen ermöglichen. Die hier gegebenen Hinweise stellen die wesentlichen Punkte heraus, auf die bei dem Einsatz von X.509-Zertifikaten geachtet werden muss. 
...
|der Version 2 haben kaum<br>auszustellen.|Anwendung gefunden. Entsprechend wird empfohlen, nur Version-3-Zertifikate|
|---|---|
|Name|Version|
...
![](markdown/tr/BSI-TR-02103/BSI-TR-02103.pdf-0010-01.png)


MUST: Kodierte Seriennummer darf maximal eine Länge von 20 Byte haben. MUST: Jede Seriennummer darf  von einer CA (d.h. für einen bestimmten Wert von issuer) nur einmal vergeben werden. 
...
|Name|Signatur|
|---|---|
|Merkmale|MUST: identisch zu Feld signatureAlgorithm außerhalb des TBS-Teils.|
...
|Ausstellernamens voraus.||
|---|---|
|Name|Aussteller (issuer-DN)|
|Merkmale|MUST: Es darf nicht leer sein.<br>MUST: Verwendung von PrintableString oder UTF8String für die Kodierung.<br>Zwei Ausnahmen liegen vor (siehe Abschnitt 4.1.2.4 [RFC 5280]).<br>Empfehlung: Die Kodierung sollte mit der Kodierung des subject-DN im<br>Zertifikat des Ausstellers binär übereinstimmen.|
...
Im Gegensatz zum Aussteller (issuer) darf das Inhaber-Feld in End-Entity-Zertifikaten leer sein, wenn der Verweis auf den Inhaber in anderen Feldern des Zertifikats enthalten ist. Beispielsweise kann ein DNS Name oder eine E-Mail-Adresse in der Subject Alternative Name Zertifikatserweiterung enthalten sein. Wenn es beabsichtigt ist, dass dieser Name die einzige Referenz auf den Zertifikatsinhaber ist, dann kann der subjectDN leer bleiben. In der Praxis ist dieser Fall jedoch sehr selten. 

|Name|Inhaber|
|---|---|
|Merkmale|MUST: Bei CA-Zertifikaten oder CRL Ausstellern muss das Feld einen Namen<br>enthalten.<br>MUST: Verwendung von PrintableString oder UTF8String für die Kodierung.<br>Drei Ausnahmen liegen vor (siehe Abschnitt 4.1.2.6 [RFC 5280]).<br>MUST: Die CA darf nicht denselben Wert für zwei verschiedene Entitäten<br>vergeben.|
...
MUST: Es darf nicht verwendet werden. 
...
Die Basic Constraints Erweiterung wird dazu verwendet, um ein Zertifikat als CA-Zertifikat oder End-EntityZertifikat erkennbar zu machen. Ein Version-3-Zertifikat darf nur dann für die Überprüfung von Signaturen anderer Zertifikate benutzen werden, wenn diese Erweiterung vorhanden ist und das Zertifikat als CAZertifikat markiert. 
...
|Merkmale|MUST: Muss in CA-Zertifikaten vorhanden sein.<br>MUST: In CA-Zertifikaten muss das Feld cA vorhanden sein und den Wert true<br>haben.|
...
|Merkmale|MUST: Für CA-Zertifikate (Wert: keyCertSign) und CRL-Aussteller (Wert:<br>cRLSign).<br>MUST: Wenn die Erweiterung im Zertifikat vorkommt, ist mindestens eine<br>explizite Verwendung gesetzt.<br>Empfehlung: Weder encipherOnly noch decipherOnly sollten gesetzt werden.|
...
Diese Erweiterung, manchmal abgekürzt als „SAN“, fügt dem Zertifikat weitere Namen des Inhabers hinzu. Beispiele sind die E-Mail-Adresse des Inhabers, um E-Mail-Verschlüsselung und -Signatur zu unterstützen, und der Domain-Name eines Servers für TLS. Die ausstellende CA muss sich davon überzeugen, dass die SANEinträge im ausgestellten Zertifikat tatsächlich dem Zertifikatsinhaber gehören und unter seiner Kontrolle sind. Zum Beispiel muss eine CA, die ein Zertifikat mit einem Domain-Name-Eintrag in der SAN ausstellt, überprüfen, ob der Antragsteller tatsächlich der Domain-Inhaber ist. 

|Name|Subject Alternative Name (SAN)|
|---|---|
|OID|2.5.29.17|
|Kritisch|MUST: kritisch, wenn leeres subject<br>SHOULD: sonst nicht kritisch|
...
|[RFC 2585] und LDAP [RFC|4516]) sein.|
|---|---|
|Name|CRL Distribution Points|
|OID|2.5.29.31|
|Kritisch|SHOULD: nicht kritisch|
|Spezifikation|Abschnitt 4.2.1.13 in [RFC 5280]|
|Merkmale|RECOMMENDS: Anwesenheit in Zertifikaten.<br>Empfehlung: Nichtverwendung von reasons.<br>MUST: Es ist nicht erlaubt, diese Erweiterung leer zu lassen oder nur das Feld<br>reasons zu setzen, mindestens ein weiteres Feld muss gesetzt werden.<br>MUST: Bei indirekten Sperrlisten muss der Name des Ausstellers der CRL im<br>Feld cRLIssuer eingetragen werden.<br>MUST: Bei direkten Sperrlisten darf der Name des Ausstellers nicht eingetragen<br>werden. In diesem Fall muss der Verteilungspunkt der CRL eingetragen<br>werden.<br>MUST: Die Kodierung des Namens des Ausstellers (cRLIssuer) muss mit der im<br>Feld issuer der CRL übereinstimmen.<br>MUST: Mindestens ein in dieser Erweiterung enthaltener Distribution Point<br>verweist auf einer CRL, welche alle Sperrgründe abdeckt.|
...
|Merkmale|MUST: Es darf nicht zweimal der gleiche OID eingetragen werden.<br>RECOMMENDS: qualifier sollten nicht verwendet werden.|
...
Diese Erweiterung, abgekürzt mit „AKI“, dient dazu, Anwendungen die Konstruktion des Zertifizierungspfades zu erleichtern. Sie enthält Informationen, mit denen das Ausstellerzertifikat des zu prüfenden Zertifikats zweifelsfrei bestimmt werden kann. Der Wert dieser Erweiterung kann bis zu drei Komponenten haben: • keyIdentifier: Ein aus dem öffentlichen Schlüssel des Ausstellers abgeleiteter Prüfwert. Dieser ist identisch mit dem Wert der Subject Key Identifier Erweiterung im Ausstellerzertifikat. 
...
|Feld|Spezifikation|Spezifikation|
|---|---|---|
...
|Spezifikation|Abschnitt 4.2.1.9 in [RFC 5280]|
|Merkmale|MUST: Muss vorhanden sein.<br>MUST: Feld cA muss vorhanden sein und den Wert true haben.|
...
|Spezifikation|Abschnitt 4.2.1.3 in [RFC 5280]|
|Merkmale|MUST: Die passenden Verwendungszwecke müssen eingetragen sein.<br>Im Falle der Ausstellung von Zertifikaten: keyCertSign<br>Im Falle der Ausstellung von Sperrlisten: cRLSign<br>Im Falle der Signatur von OCSP-Antworten: digitalSignature|
...
|Nicht erlaubt ist die Verwe|ndung der speziellen anyPolicy in dieser Erweiterung.|
|---|---|
|Name|Policy Mappings|
|OID|2.5.29.33|
|Kritisch|SHOULD: als kritisch markiert|
|Spezifikation|Abschnitt 4.2.1.5 in [RFC 5280]|
|Merkmale|MUST: Es ist keine Abbildung von oder zu anyPolicy erlaubt.|
...
Diese Erweiterung schränkt den Namensraum für die Namen der Inhaber aller im Pfad nachfolgenden Zertifikate ein. Nicht betroffen von der Einschränkung sind selbst ausgestellte[1] (self-issued) Zertifikate. Auf diese Weise ist es einem Aussteller möglich, solche Zertifikate für die eigenen Schlüssel unabhängig von den Einschränkungen auszustellen, die für Zertifikate gelten, die für andere Entitäten ausgestellt werden. 
...
> 1 Ein selbst ausgestelltes (self-issued) Zertifikat ist dadurch gekennzeichnet, dass dessen Felder issuer und subject inhaltlich übereinstimmen. In einem Zertifizierungspfad können z.B. dann selbst ausgestellte Zertifikate vorkommen, wenn ein Aussteller eigene Zertifikate mit verschiedenen Schlüssellängen sich gegenseitig signieren lässt. 
...
|Authority Information Access|Empfehlung: enthalten|MUST: nicht kritisch|
|Subject Alternative Name|Empfehlung: enthalten|MUST: kritisch, falls subject leer<br>ist|
...
Um das TLS Server Zertifikat als ein End-Entity-Zertifikat auszuweisen, wird empfohlen, diese Erweiterung zu verwenden. Wenn die Erweiterung verwendet wird, muss das Feld cA den Wert false haben[2] und das Feld pathLenConstraint  sollte nicht vorhanden sein. 
...
|Kritisch|Darf sowohl als kritisch als auch als nicht-kritisch markiert sein.|
|Spezifikation|Abschnitt 4.2.1.9 in [RFC 5280]|
|Merkmale|MUST: wenn die Erweiterung vorhanden ist, dann muss das Feld cA den Wert<br>false haben.<br>Empfehlung: pathLenConstraint nicht vorhanden.|
...
|grundsätzlich abweisen.||
|---|---|
|Name|Subject Alternative Name (SAN)|
|OID|2.5.29.17|
|Kritisch|MUST: kritisch, wenn leeres subject<br>SHOULD: sonst nicht kritisch|
|Spezifikation|Abschnitt 4.2.1.6 in [RFC 5280], [RFC 6125]|
|Merkmale|MUST: Enthält DNS Namen (in dNSName) oder die IP-Adressen (in iPAddress)<br>des Servers.<br>MUST: Die individuellen Werte und deren Kodierung müssen konform zu den<br>entsprechenden Spezifikationen sein.<br>MUST: Der Wert der Erweiterung darf nicht leer sein.|



2 Weil false der Defaultwert ist und die Zertifikate DER kodiert sind, darf dieser Wert nicht kodiert werden (siehe Abschnitt 11.5 in [X.690]). 
...
|Merkmale|SHOULD: Sollte gesetzt werden.<br>MUST: Wenn die Erweiterung im Zertifikat vorkommt, ist im Fall von TLS 1.2<br>mindestens eine der Verwendungen keyEncipherment, keyAgreement oder<br>digitalSignature gesetzt, im Fall von TLS 1.3 ist digitalSignature gesetzt.|
...
|---|---|
|Spezifikation|Abschnitt 4.2.1.12 in [RFC 5280]|
|Merkmale|SHOULD: Die Erweiterung sollte gesetzt werden.<br>Empfehlung: Wenn vorhanden, muss mindestens der Zweck serverAuth mit<br>dem OID 1.3.6.1.5.5.7.3.1 gesetzt sein.|
...
|Subject Key Identifier|Empfehlung: enthalten|MUST: nicht kritisch|
|Authority Information Access|Empfehlung: enthalten|MUST: nicht kritisch|
|Subject Alternative Name|Optional|MUST: kritisch, falls subject<br>leer ist|
|Extended Key Usage|Empfehlung: enthalten|Empfehlung: nicht kritisch|
...
Um TLS Client Zertifikate als End-Entity-Zertifikate auszuweisen, wird empfohlen, diese Erweiterung zu verwenden. Wenn die Erweiterung verwendet wird, muss das Feld cA den Wert false haben[3] und das Feld pathLenConstraint  sollte nicht vorhanden sein. 
...
- 3 Weil false der Defaultwert ist und die Zertifikate DER kodiert sind, darf dieser Wert nicht kodiert werden (siehe Abschnitt 11.5 in [X.690]). 
...
|---|---|
|Merkmale|SHOULD: Sollte gesetzt werden.<br>SHOULD: Im Fall von TLS 1.2 gilt, dass wenn die Erweiterung im Zertifikat<br>vorkommt, mindestens eine der Verwendungen keyAgreement oder<br>digitalSignature gesetzt ist.<br>MUST: Wenn die Erweiterung im Zertifikat vorkommt, ist mindestens ein<br>Schlüsselverwendungszweck gesetzt.|
...
|Kritisch|Darf sowohl als kritisch als auch als nicht-kritisch markiert sein.|
|Spezifikation|Abschnitt 4.2.1.12 in [RFC 5280]|
|Merkmale|SHOULD: Die Erweiterung sollte gesetzt werden.<br>Empfehlung: Wenn vorhanden, muss mindestens der Zweck clientAuth mit<br>dem OID 1.3.6.1.5.5.7.3.2 gesetzt sein.|
...
|Subject Key Identifier|Empfehlung: enthalten|MUST: nicht kritisch|
|Authority Information Access|Empfehlung: enthalten|MUST: nicht kritisch|
|Subject Alternative Name|Empfehlung: enthalten|MUST: kritisch, falls subject leer ist|
|Extended Key Usage|Empfehlung: enthalten|Empfehlung: nicht kritisch|
...
Um S/MIME Zertifikate als End-Entity-Zertifikate auszuweisen wird empfohlen, diese Erweiterung zu verwenden. Wenn die Erweiterung verwendet wird, muss das Feld cA den Wert false haben[4] und das Feld pathLenConstraint  sollte nicht vorhanden sein. 
...
|Erweiterung verwendet we<br>werden.|rden. Die E-Mail-Adresse muss dabei in einem Feld des Typs rfc822Name kodiert|
|---|---|
|Name|Subject Alternative Name (SAN)|
|OID|2.5.29.17|
|Kritisch|MUST: kritisch, wenn leeres subject<br>SHOULD: sonst nicht kritisch|
...
> 4 Weil false der Defaultwert ist und die Zertifikate DER kodiert sind, darf dieser Wert nicht kodiert werden (siehe Abschnitt 11.5 in [X.690]). 
...
|Kritisch|Darf sowohl als kritisch als auch als nicht-kritisch markiert sein.|
|Spezifikation|Abschnitt 4.2.1.12 in [RFC 5280]|
|Merkmale|SHOULD: Die Erweiterung sollte gesetzt werden.<br>MUST: Wenn vorhanden, muss mindestens der Zweck emailProtection mit<br>dem OID 1.3.6.1.5.5.7.3.4 gesetzt sein.|
...
|Subject Key Identifier|Empfehlung: enthalten|MUST: nicht kritisch|
|Authority Information Access|Empfehlung: enthalten|MUST: nicht kritisch|
|Subject Alternative Name|MUST: enthalten, falls als ID<br>nicht „DN“ benutzt wird|MUST: kritisch, falls subject leer ist|
...
Um IPsec Zertifikate als End-Entity-Zertifikate auszuweisen, wird empfohlen, diese Erweiterung zu verwenden. Wenn die Erweiterung verwendet wird, muss das Feld cA den Wert false haben[5] und das Feld pathLenConstraint  sollte nicht vorhanden sein. 
...
|Merkmale|SHOULD: Enthält DNS Namen (in dNSName) oder die IP-Adressen (in<br>iPAddress) des Servers oder einen rfc822Name<br>MUST: Die individuellen Werte und deren Kodierung müssen konform zu den<br>entsprechenden Spezifikationen sein.<br>MUST: Der Wert der Erweiterung darf nicht leer sein.<br>MUST: Es dürfen keine Wildcard Namen verwendet werden.<br>MUST: Bei IP-Adressen dürfen keine Adressbereiche ausgewiesen werden,<br>sondern nur vollständig festgelegte Adressen.|



Tabelle 49: Subject Alternative Name Erweiterung in IPsec Zertifikaten – Übersicht 

> 5 Weil false der Defaultwert ist und die Zertifikate DER kodiert sind, darf dieser Wert nicht kodiert werden (siehe Abschnitt 11.5 in [X.690]). 
...
|Merkmale|SHOULD: Sollte gesetzt werden.<br>MUST: Wenn die Erweiterung im Zertifikat vorkommt, ist mindestens eine der<br>Verwendungen nonRepudiation oder digitalSignature gesetzt.<br>Empfehlung: Im Fall von IKE oder IKEv2 sollte keine andere Verwendung<br>außer nonRepudiation oder digitalSignature gesetzt sein.|
...
|Bezeichnung|Pfadkonstruktion|Pfadkonstruktion|
|---|---|---|
...
|Zertifikatserweiterungen|Authority Key Identifier|Überprüfung der Verkettung über<br>Ausstellerschlüssel für jeden<br>möglichen Pfadkandidaten|
...
|Bezeichnung|Prüfung der Zertifikatsversion|Prüfung der Zertifikatsversion|
|---|---|---|
...
|Zertifikatserweiterungen|Feld extensions|Falls das Feld extensions vorhanden ist:<br>Prüfung ob dieses von der ermittelten<br>Version unterstützt wird|
|Benutzerparameter|–||
|Eigenschaften des<br>Zertifizierungspfads|–||
...
|Bezeichnung|Prüfung der kritischen Zertifikatserweiterungen|Prüfung der kritischen Zertifikatserweiterungen|
|---|---|---|
...
|Zertifikatserweiterungen|Feld extensions|Falls das Feld extensions vorhanden ist,<br>muss die Prüfung der Kritikalität und<br>Unterstützung durch die<br>Implementierung für jede darin<br>enthaltene Erweiterung durchgeführt<br>werden|
|Benutzerparameter|–||
|Eigenschaften des<br>Zertifizierungspfads|–||
|Sonstige Eingabeparameter|–||
...
|Bezeichnung|Prüfung der Namensverkettung|Prüfung der Namensverkettung|
|---|---|---|
|Beschreibung|Es muss geprüft werden, ob die Verkettung von subject und issuer<br>aller Zertifikate im Pfad korrekt ist.||
|Anzuwenden auf|alle Zertifikate im Pfad||
|Referenzen|Abschnitt 4.1.2.4 in [RFC 5280]||
|Typ des Eingabeparameters|Verwendete Parameter|Verwendung|
|Basisfelder des Zertifikats|subject, issuer|Prüfung der korrekten Namensverkettung|
...
|Bezeichnung|Prüfung der Zertifikatssignaturen|Prüfung der Zertifikatssignaturen|
|---|---|---|
...
|Typ des Eingabeparameters|Verwendete Parameter|Verwendung|
|Basisfelder des Zertifikats|signature, signatureValue,<br>subjectPublicKeyInfo|Prüfung, ob die Signatur in signatureValue<br>mit dem in signature angegebenen<br>Verfahren und dem öffentlichen Schlüssel<br>des Ausstellers in dessen<br>subjectPublicKeyInfo verifiziert werden<br>kann|
|Zertifikatserweiterungen|–||
...
|Bezeichnung|Prüfung der zeitlichen Gültigkeit|Prüfung der zeitlichen Gültigkeit|
|---|---|---|
...
|Bezeichnung|CA-Zertifikatsprüfung|CA-Zertifikatsprüfung|
|---|---|---|
...
Für jedes CA-Zertifikat im Pfad außer dem Zielzertifikat muss geprüft werden, ob es sich um ein gültiges CAZertifikat handelt. Für die jeweiligen X.509 Versionen müssen jeweils die aufgelisteten Prüfungen durchgeführt werden. Schlägt eine der Prüfungen fehl, so ist die Pfadvalidierung fehlgeschlagen. 

1. Prüfung von Version 1 oder Version 2 Zertifikaten. Sofern die Implementierung diese Version von X.509 Zertifikaten unterstützt, muss auf durch den Einsatzkontext vorgegebenen Wegen sichergestellt werden, dass es sich um ein CA-Zertifikat handelt. Ein möglicher Ansatz ist es, nur solche Version-1- oder Version2-Zertifikate als CA-Zertifikate zu klassifizieren, die zuvor über einen kryptographischen Hashwert des Zertifikats als solche in der Implementierung hinterlegt wurden. 
...
2. Prüfung von Version-3-Zertifikaten. Es müssen die nachfolgend aufgelisteten Eigenschaften geprüft werden. 
   ...
   - b. In der Basic Constraints Erweiterung ist das Feld cA auf den Wert true gesetzt. 

   - c. Falls das Zertifikat eine Key Usage Erweiterung enthält, dann ist in dieser der Verwendungszweck keyCertSign gesetzt. 

## 8.4.1.9 Prüfung der Pfadlängenbeschränkung 

|Bezeichnung|Prüfung der Pfadlängenbeschränkung|Prüfung der Pfadlängenbeschränkung|
|---|---|---|
...
Tabelle 59: Prüfung der Pfadlängenbeschränkung – Übersicht Um die Erfüllung aller Pfadlängenbeschränkungen in dem zu validierenden Pfad zu prüfen, ist wie folgt vorzugehen. 
...
|8.4.1.10<br>Überprüfung|der Zertifikatsrichtlinien|der Zertifikatsrichtlinien|
|---|---|---|
...
|Benutzerparameter|initial-explicit-policy|Falls initial-explicit-policy den Wert<br>„wahr“ hat, muss user-initial-policy-set<br>mindestens eine Policy enthalten, die von<br>jedem Zertifikat im Pfad erfüllt wird|
||user-initial-policy-set||
|Eigenschaften des<br>Zertifizierungspfads|–||
|Sonstige Eingabeparameter|–||
...
Die in Abschnitt 8.4.1 beschriebene Pfadvalidierung unterstützt nicht die Verarbeitung der Name Constraints Erweiterung. Falls diese Funktionalität in einer Implementierung benötigt wird, so müssen die entsprechenden Schritte des formalen Algorithmus in [RFC 5280] umgesetzt werden. Diese umfassen die Schritte (b) und (c) unter Abschnitt 6.1.3 sowie (g).(1) und (g).(2) unter Abschnitt 6.1.4 jeweils in [RFC 5280]. Ferner muss die korrekte Initialisierung der beteiligten formalen Variablen permitted_subtrees und excluded_subtrees zu Beginn der Pfadvalidierung erfolgen. Dabei kann auch eine Unterstützung der relevanten Benutzerparameter entsprechend Tabelle 52 umgesetzt werden. 
...
Im Falle von Sperrlisten gibt es noch die Unterscheidung zwischen vollständigen Sperrlisten und DeltaSperrlisten (Delta CRLs). In allen Fällen erfolgt die Identifikation eines Zertifikats in den Sperrinformationen über dessen Ausstellernamen (issuer) und seine Seriennummer (serialNumber). 
...
|Bezeichnung|Überprüfung des Revokationsstatus mittels vollständiger Sperrlisten,<br>die alle Sperrgründe abdecken|Überprüfung des Revokationsstatus mittels vollständiger Sperrlisten,<br>die alle Sperrgründe abdecken|
|---|---|---|
...
|Zertifikatserweiterungen|CRL Distribution Points|Bestimmung der Speicherorte von<br>aktuellen Sperrlisten, die in der Prüfung<br>verwendet werden bzw. Zuordnung einer<br>Sperrliste zu einem Distribution Point|
...
Sperrstatus Algorithmus ersetzt. Statt wie in diesen Schritten spezifiziert, die Kombination der Sperrgründe von den einzelnen verarbeiteten Sperrlisten zu berechnen, wird stattdessen 
   ...
   - und jede Sperrliste von der Verarbeitung ausgeschlossen, deren zugeordnetes DistributionPointElement in der CDP im Zertifikat nur eine Untermenge aller möglichen Sperrgründe spezifiziert. Eine Möglichkeit dies umzusetzen besteht darin, auf den Zugriff auf eine solche Sperrliste zu verzichten. 
...
2. Vom OCSP-Responder signierte Antworten können von dem gleichen Schlüssel signiert sein wie dem, welcher das geprüfte Zertifikat ausstellt. Andernfalls muss der Zertifikatsaussteller diese Aufgabe an einen anderen Schlüssel delegieren, indem er ein entsprechendes OCSP-Signer-Zertifikat ausstellt. Ein solches Zertifikat ist dadurch gekennzeichnet, dass es die Extended Key Usage Erweiterung mit dem Zweck idkp-OCSPSigning enthält (Abschnitt 4.2.2.2, [RFC 6960]). 

3. Falls die OCSP-Antworten nicht direkt von dem Zertifikatsaussteller signiert werden, muss prinzipiell auch der Revokationsstatus des OCSP-Signer-Zertifikats geprüft werden. Hierbei gibt es folgende Möglichkeiten (siehe [RFC 6960], Abschnitt 4.2.2.2.1): 
   ...
   - d. Falls keine der in den vorhergehenden Punkten aufgeführten Kriterien im OCSP-SignerZertifikat erfüllt sind, ist es der Anwendung überlassen, ob und wie sie dessen Revokationsstatus prüft. 
...
1. Falls in dem Zertifikat die Subject Alternative Name Erweiterung enthalten ist und mindestens einen Eintrag 
   ...
   - c. vom Typ otherName, welcher einen SRVName[6] beinhaltet, 
...
> 6 Ein SRVName bezieht sich auf einen SRV DNS Resource Record, welcher Dienste zu einer Domain auflistet [RFC 2782]. Die Verwendung von SRVName in X.509 Zertifikaten wird in [RFC 4985] definiert. 
...
> 7 Eine nicht offizielle, aber weithin genutzte Liste mit solchen Domains, deren Vollständigkeit nicht garantiert ist, ist unter https://publicsuffix.org/ verfügbar. 
...
|DN|subject|ID muss binär mit dem Inhalt von subject<br>übereinstimmen.<br>Das Feld subject darf nicht leer sein.|-|
...
|Zertifikatserweiterung|Pfadkonstruktion (Abschnitt<br>8.3)|Pfadvalidierung<br>mit<br>Mindestfunktionalität<br>(Abschnitt 8.4.1)|Erweiterte<br>Pfadvalidierung<br>„Policy-Verarbeitung“<br>(Abschnitt 8.4.2)|Erweiterte<br>Pfadvalidierung<br>„Name<br>Constraints“<br>(Abschnitt 8.4.3)|Revokationsprüfung<br>mit<br>Mindestfunktionalität<br>(Abschnitt 8.5.2)|Revokationsprüfung<br>mit<br>Delta-Sperrlisten (Abschnitt<br>8.5.4)|Revokationsprüfung<br>mit<br>OCSP (Abschnitt 8.5.5)|Anwendungsspezifische<br>Prüfung|
|---|---|---|---|---|---|---|---|---|
|Basic Constr.|–|ja|–|–|ja|–|–|–|
|Key Usage|–|ja|–|–|ja|–|–|ja|
|Ext. Key Usage|–|–|–|–|–|–|ja|ja|
...
In diesem Abschnitt wird erläutert, wie das Testwerkzeug „Certification Path Validation Test Tool“[ 11] (CPT) dazu verwendet werden kann, um Implementierungen der Zertifizierungspfadvalidierung hinsichtlich der in diesem Dokument spezifizierten Teilprüfungen zu testen. 
...
https://www.bsi.bund.de/DE/Themen/Kryptografie_Kryptotechnologie/Kryptografie/CPT/cpt_node.ht ml 
...
[CPT-S] Certification Path Validation Test Tool — Test Specification, Version 1.1 vom 23.10.2018. - - - https://www.bsi.bund.de/SharedDocs/Downloads/EN/BSI/CPT/CPT Test Tool Specification_v1_1.pdf 

- [CPT-T] Certification Path Validation Test Tool, https://www.bsi.bund.de/CPT [RFC 2119] S. Bradner: Key words for use in RFCs to Indicate Requirement Levels (RFC 2119), März 1997 [RFC 2585] R. Housley, P. Hoffman: Internet X.509 Public Key Infrastructure Operational Protocols: FTP and HTTP (RFC 2585), Mai 1999 
...
- [X.509/16] ITU-T: SERIES X: DATA NETWORKS, OPEN SYSTEM COMMUNICATIONS AND SECURITY / Directory, Information technology – Open Systems Interconnection – The Directory: Publickey and attribute certificate frameworks, Oktober 2016 
