...
## <span id="page-5-1"></span>1.1. Changelog 1.4.1
...
In contrast to the specifications given in [\[BSI-TR-03129-3\]](#page-62-3), this Web Service SHALL only support synchro nous communication: Hence, the input paramter callbackIndicator MUST be set to callback\_not\_possi ble. Furthermore parameters messageID and responseURL MUST be omitted.
...
### <span id="page-12-0"></span>3.3.1. GetDocumentSignerInformation
...
The parameter certificateChainValidity contains validity information about the signer certificate and the corresponding CA certificate. The parameter has the type certificateChainValidityType and its structure is given in [Table 3.5.](#page-13-1)
...
<span id="page-16-4"></span>**Parameter Description Input** keyCAR REQUIRED A [Certificate Authority Reference \(CAR\)](#page-60-22) obtained from an [eMRTD.](#page-60-6) This is a reference to one of the public keys which can be used by the [eMRTD](#page-60-6) for the verification of [CV](#page-60-16) certificates. **Output**
...
A [TCC](#page-61-1) SHALL implement a Web Service towards the role ["DV"](#page-60-12) supporting the message SendSignerList ( [Sec](#page-22-0) [tion 5.2.2](#page-22-0)).
...
The message GetSignerList is used to get a signed list of signer certificates. Its parameters are given in [Ta](#page-22-2) [ble 5.2.](#page-22-2)
...
The use of [VDS](#page-61-3) for residence permits is described in [\[BSI-TR-03137-1\].](#page-62-14)
...
The parameter vaccinationEventV1 is of type VDSNC.vaccinationEventV1Type. Its structure is given in [Ta](#page-39-0) [ble 6.28](#page-39-0).
...
The vaccinationDetails of a version 2 vaccination event vaccinationEventV2 use the structure given in [Ta](#page-40-0) [ble 6.31](#page-40-0).
...
The structure of a proof of recovery, i.e. proofOfRecovery, is according to type VDSNC.proofOfRecoveryMessage Type as given in [Table 6.32.](#page-40-1)
...
A test result, i.e. the parameter testResult, is given by the structure given in [Table 6.33.](#page-41-2)
...
| dateOfFirstPositiveNAATTest<br>Result | See [ICAO TR VDS-NC]. |
|                                       | REQUIRED              |
|                                       | See [ICAO TR VDS-NC]. |
|                                       |                       |
...
## <span id="page-41-0"></span>6.3. EUCWT

The EU specifies digital seals containing health certificates (see [\[EU DCC Imp. Decision\]](#page-62-16)). Those digital seals contain a [CBOR Web Token \(CWT\)](#page-60-29) (see [\[RFC8392\]\) which is a cryptographically signed data structure \(her](#page-63-1) einafter called EUCWT).
...
The structure of a EUCWT is given in [Table 6.34.](#page-41-1)

**Table 6.34** EUCWT.EUCWTType definition

The parameter header has the type EUCWT.headerType and its structure is given in [Table 6.35.](#page-41-3)
...
The parameter payload has the type EUCWT.payloadType and its structure is given in [Table 6.36](#page-42-0).
...
#### The EUDCC is specified in [Table 6.37.](#page-42-1)
...
The parameter name has the type EUCWT.personNameType and its structure is given in [Table 6.38](#page-43-0).
...
An EUDCC can contain information about a vaccination. The structure of the parameter vaccinationEntry is given in [Table 6.39.](#page-43-1)
...
An EUDCC can contain information about a test. The structure of the parameter testEntry is given in [Table 6.40](#page-44-0).
...
An EUDCC can contain information about a recovery entry. The structure of the parameter recoveryEntry is given in [Table 6.41.](#page-45-0)
...
## <span id="page-46-0"></span>7. Defect Lists for ePassport Application

A Defect is a production error affecting large number of documents. The withdrawal already issued do cuments is at best impractical and impossible if foreign documents are affected. Defects are identified by the [DS](#page-60-15) certificate(s) used to produce defect documents.

[Defect Lists \(DFLs\)](#page-60-30) are errata that inform about defects and provide corrigenda to fix errors if possible. [DFLs](#page-60-30) are provided as Signed Data according to [\[RFC5652\]](#page-63-2) and SHALL be using the profile and list content description as specified in [\[BSI-TR-03129-3\].](#page-62-3)

This chapter defines [DFLs](#page-60-30) for the handling of Defects in the context of official document checks making use of the ePassport Application on a chip. Defects relating to the eID Application are defined in [\[BSI-TR-03129-3\]](#page-62-3).

Defect handling procedures for [IS](#page-60-5) are defined in [\[BSI-TR-03135-1\].](#page-62-8)

## <span id="page-46-2"></span><span id="page-46-1"></span>7.1. Defect List Format

 hashAlg OBJECT IDENTIFIER, defects SET OF Defect

An inspection system compliant with this [TR](#page-61-0) SHALL support version 1 and version 2 of the [DFL](#page-60-30) format. A [DFL](#page-60-30) SHALL be provided as [Basic Encoding Rules \(BER\)](#page-60-31) encoded SignedData with content type DefectList identified by id-DefectList (see [\[BSI-TR-03129-3\]](#page-62-3)):
...
```
Version 2 of the [DFL](#page-60-30) format introduces an OPTIONAL description field. It can contain a human readable description of the class of a Defect and additional information and notes on the Defect or affected documents. A Defect is specified as given below:
...
```
The type SignerIdentifier is defined in [\[RFC5652\]](#page-63-2) as follows:
...
There are two options to reference a [DS](#page-60-15) certificate in a [DFL](#page-60-30):

- 1. by the distinguished name of the [CSCA](#page-60-14) (issuer) and the serial number of the [DS](#page-60-15) certificate
- 2. by the [Subject Key Identifier \(SKI\)](#page-60-32) of the [DS](#page-60-15) certificate

If neither option uniquely identifies the [DS](#page-60-15) certificate (e.g. due to a defect in the certificate), the hash of the [DS](#page-60-15) certificate SHALL additionally be included. The hash function to be used is indicated in DefectList.

### <span id="page-47-0"></span>7.2. Defect List Signer

A defect list SHALL be signed by a [DFL](#page-60-30) Signer. A [DFL](#page-60-30) Signer certificate SHOULD include the following [OID](#page-60-26) inside the extendedKeyUsage extension:
...
```
This requirement will become mandatory in future versions of this [TR](#page-61-0).

Any system that is processing a [DFL](#page-60-30) with the intent to directly or indirectly apply defects from that [DFL](#page-60-30) SHALL check whether the [DFL](#page-60-30) has a valid signature from a [DFL](#page-60-30) Signer by using a [DFL](#page-60-30) Signer certificate. If the signature is invalid, the system SHALL NOT use that [DFL.](#page-60-30) If the signature is not from a [DFL](#page-60-30) Signer, the system SHALL NOT use that [DFL](#page-60-30).

### <span id="page-47-1"></span>7.3. Defect Categories
...
The private key of the [DS](#page-60-15) certificate might be compromised (e.g. revoked by a [Certificate Revocation List](#page-60-33) [\(CRL\)\)](#page-60-33).

Note that this defect is equivalent to an issued [CRL](#page-60-33) containing the [DS](#page-60-15) certificate. This type of defect can be used to revoke foreign [DS](#page-60-15) certificates independently from a [CRL](#page-60-33).
...
The [DS](#page-60-15) certificate might be malformed and therefore might not be decoded correctly. Instead a replacement certificate is provided. *Note that this certificate is not necessarily signed by the corresponding* [CSCA](#page-60-14)*. Replacement certificates MAY be self-signed.*
...
The parameter ReplacementCertificate contains a certificate (Certificate, cf. [\[RFC5280\]\)](#page-62-17) as replacement for the [DS](#page-60-15) certificate.
...
The validity period of the [DS](#page-60-15) certificate or [CSCA](#page-60-14) certificate might be incorrect.
...
The signer identifier inside the EF.SOD might be wrong, hence there might be no matching [DS](#page-60-15) certificate in the security object.
...
The country element of the issuer's distinguished name inside the [DS](#page-60-15) certificate might be missing or incorrect. (e.g. using an [ICAO](#page-60-27) 3-letter code instead of an [ICAO](#page-60-27) 2-letter code , see [\[ICAO Doc 9303 p3\],](#page-62-18) or using the wrong value).
...
The Card Security Object might be incorrectly encoded. A corrected Card Security Object (see [\[BSI-](#page-62-19)[TR-03110-3\]](#page-62-19)) is provided and SHOULD be used.
...
The [DS](#page-60-15) certificate might be incorrectly encoded or malformed and unusable. Compared to the De fect id-CertReplaced there is no replacement certificate provided. Additional information about this Defect is provided with the parameter DSMalformedInformation.
...
```
## <span id="page-53-3"></span><span id="page-53-0"></span>8. Signed Data Profiles for Signer Lists

Signer Lists are provided as Signed Data according to [\[RFC5652\]](#page-63-2) and SHALL be using the profile as defined in [\[BSI-TR-03129-3 Annex\].](#page-62-20) The value eContentType SHALL be determined according to [Table 8.1.](#page-53-4)
...
Each list SHALL be provided as [BER](#page-60-31) encoded Signed Data, i.e. each list is a signed set of x.509 certificates in [BER](#page-60-31) encoding.
...
```
Each list is defined in the bsi-de [OID](#page-60-26) branch:
...
```
## <span id="page-53-1"></span>8.1. Document Signer List Format
...
```
## <span id="page-53-2"></span>8.2. EU Health Seal Signer List Format
...
```
## <span id="page-54-0"></span>8.3. ICAO Health Seal Signer List Format
...
```
### <span id="page-54-1"></span>8.4. Sovereign Seal Signer List Format
...
```
### <span id="page-54-2"></span>8.5. Signer List Signers

A Signer List SHALL be signed by an eligible Signer List Signer. A Signer List Signer certificate SHALL include an [OID](#page-60-26) in the extendedKeyUsage extension for every type of Signer List that the Signer List Signer shall be entitled to sign. The [OIDs](#page-60-26) to use are given in [Table 8.2.](#page-54-3) An Signer List Signer certificate MAY include more than one of those [OIDs](#page-60-26) inside its extendedKeyUsage extension to allow the Signer List Signer to use one private key to sign different signer list types.

<span id="page-54-3"></span>

| Signer List Type           | OID                                                                                                                                                   |
|----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| Document Signer List       | id-documentSignerListSigningKey OBJECT IDENTIFIER ::= { bsi<br>de applications(3) id-npkd(11) id-certificateExchange(1) id<br>documentSigner(4) 2 }   |
| EU Health Seal Signer List | id-euHealthSealSignerListSigningKey OBJECT IDENTIFIER ::= { bsi<br>de applications(3) id-npkd(11) id-certificateExchange(1) id<br>euHealthSeal(2) 2 } |

| Signer List Type             | OID                                                                                                                                                       |
|------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| ICAO Health Seal Signer List | id-icaoHealthSealSignerListSigningKey OBJECT IDENTIFIER ::= { bsi<br>de applications(3) id-npkd(11) id-certificateExchange(1) id<br>icaoHealthSeal(3) 2 } |
| Sovereign Seal Signer List   | id-sovereignSealSignerListSigningKey OBJECT IDENTIFIER ::= { bsi<br>de applications(3) id-npkd(11) id-certificateExchange(1) id<br>sovereignSeal(1) 2 }   |
...
## <span id="page-56-2"></span><span id="page-56-0"></span>Appendix A. Infrastructures for Terminals

A terminal is a reader component that contains the [PCD](#page-60-7) used for communication between terminal and [MRTD](#page-60-2).

More than one reader component may be part of a single terminal. For example, all readers at the border control of an airport can be part of the same [IS.](#page-60-5)

Even if the terminal consists of more than one reader, the terminal has only one identity. Regardless at which reader of a terminal a [MRTD](#page-60-2) is presented, the identical key pair and [CV](#page-60-16) certificates are used for [TA](#page-61-2) between the terminal and the [MRTD.](#page-60-2) From this point of view, the [TA](#page-61-2) is done rather between the terminal and the [MRTD](#page-60-2) than between the single reader and the [MRTD.](#page-60-2)

## <span id="page-56-1"></span>A.1. Integrated Terminals

An overview over the architecture of an integrated terminal is given in [Figure A.1](#page-57-1).

An integrated terminal has only one reader. The [HSM](#page-60-9) containing the private key of the terminal is physically part of the reader.

All communication between the [HSM](#page-60-9) and the presented [MRTD](#page-60-2) for performing the [TA](#page-61-2) is done in the reader internally or between the [PCD](#page-60-7) and the [MRTD.](#page-60-2) No online connection to other components is needed for the [TA](#page-61-2) .

The reader (or rather the terminal) needs an online connection to the [DV](#page-60-12) to which it is associated. It must be connected only temporarily for requesting a new [CV](#page-60-16) certificate or for downloading [CV](#page-60-16) certificates of the [DV](#page-60-12) or of [CVCAs.](#page-60-10)

The disadvantage of this architecture is, that a stolen reader can be used to perform [TA](#page-61-2) at least as long as the current [CV](#page-60-16) certificate is valid. For this reason, organisational and/or technical security arrangements must be implemented within the intended operational environment of the terminal to avoid unauthorised access to the reader.

<span id="page-57-1"></span>![](_page_57_Figure_1.jpeg)

**Figure A.1.** Integrated terminal

## <span id="page-57-0"></span>A.2. Distributed Terminals

A distributed terminal may have more than one reader that is coordinated by a [TCC.](#page-61-1) The [HSM](#page-60-9) containing the private key of the terminal is not part of the readers. Instead the [HSM](#page-60-9) is integrated into the [TCC.](#page-61-1)

We distinguish between:

- **•** distributed terminal with local control centre (see [Section A.2.1\)](#page-57-2) and
- **•** distributed terminal with remote control centre (see [Section A.2.2\)](#page-58-1).

In a distributed terminal, all readers of a terminal have got a permanent online connection with the [TCC.](#page-61-1) Since the private key of the terminal is stored in the [HSM](#page-60-9) within the [TCC](#page-61-1), each [TA](#page-61-2) between a terminal and a [MRTD](#page-60-2) presented at one of its readers needs also some communication between the reader and the [TCC.](#page-61-1)

The communication over the online connection between the [TCC](#page-61-1) and the readers of the terminal must be secured. The actual measures to be taken to achieve the required level of security are decided by the operator of the terminal based on the operational environment. In any case it must be assured that the [TCC](#page-61-1) does not communicate with a reader which is not under the control of the operator of the terminal.

#### <span id="page-57-2"></span>A.2.1. Local Control Centre

For a distributed terminal, the connected readers do not need a (direct) online connection to the [DV](#page-60-12). Instead, the [TCC](#page-61-1) performs all communication with the [DV](#page-60-12) by requesting new [CV](#page-60-16) certificates or downloading [CV cer](#page-60-16) tificates of the [DV](#page-60-12) or the [CVCAs.](#page-60-10) For this, the online connection between the [TCC](#page-61-1) and the [DV](#page-60-12) is only tem porarily required.

Hence, the advantage of this architecture is, that a stolen reader cannot be used for [TA.](#page-61-2) Therefore, each reader can be operated easily in an insecure environment. Only for the [TCC](#page-61-1) organisational and/or technical security arrangements must be implemented within the intended operational environment of the terminal to avoid unauthorised access to the [TCC](#page-61-1) (especially to the [HSM](#page-60-9)).

<span id="page-58-0"></span>An overview over the architecture of a distributed terminal is given in [Figure A.2](#page-58-0).

![](_page_58_Figure_3.jpeg)

**Figure A.2.** Distributed terminal with local control centre

## <span id="page-58-1"></span>A.2.2. Remote Control Centre

If the [TCC](#page-61-1) cannot be operated securely in the intended operational environment of the terminal, the [TCC](#page-61-1) can also be operated in the operational environment of the [DV](#page-60-12) provider.

In this case, a permanent online connection between the [DV](#page-60-12) and each reader of the terminal is required for exchanging data during [TA.](#page-61-2)However, the communication between the [DV](#page-60-12) and the [TCC](#page-61-1) for requesting a new [CV](#page-60-16) certificate or for downloading [CV](#page-60-16) certificates from the [DV](#page-60-12) or of [CVCAs](#page-60-10) can be implemented as [DV-](#page-60-12)internal communication.
...
An overview over the architecture of a distributed terminal of this second variant is given in [Figure A.3](#page-59-0).
...
## <span id="page-60-0"></span>**List of Abbreviations**

<span id="page-60-33"></span><span id="page-60-32"></span><span id="page-60-31"></span><span id="page-60-30"></span><span id="page-60-29"></span><span id="page-60-28"></span><span id="page-60-27"></span><span id="page-60-26"></span><span id="page-60-25"></span><span id="page-60-24"></span><span id="page-60-23"></span><span id="page-60-22"></span><span id="page-60-21"></span><span id="page-60-20"></span><span id="page-60-19"></span><span id="page-60-18"></span><span id="page-60-17"></span><span id="page-60-16"></span><span id="page-60-15"></span><span id="page-60-14"></span><span id="page-60-13"></span><span id="page-60-12"></span><span id="page-60-11"></span><span id="page-60-10"></span><span id="page-60-9"></span><span id="page-60-8"></span><span id="page-60-7"></span><span id="page-60-6"></span><span id="page-60-5"></span><span id="page-60-4"></span><span id="page-60-3"></span><span id="page-60-2"></span><span id="page-60-1"></span>
...
<span id="page-61-4"></span><span id="page-61-3"></span><span id="page-61-2"></span><span id="page-61-1"></span><span id="page-61-0"></span>
...
## <span id="page-62-0"></span>Bibliography

- <span id="page-62-6"></span>[BSI-TR-03110-1] *BSI-TR-03110: Advanced Security Mechanisms for Machine Readable Travel Documents and eIDAS Token Part 1 – eMRTDs with BAC/PACEv2 and EACv1.* 2015.
- <span id="page-62-19"></span>[BSI-TR-03110-3] *BSI-TR-03110: Advanced Security Mechanisms for Machine Readable Travel Documents and eIDAS Token Part 3: Common Specifications.* 2016.
- <span id="page-62-2"></span>[BSI-TR-03129-1] *BSI-TR-03129: Protocols for the Management of Certificates and CRLs in Public-Key-Infra structures (PKIs) Part 1: Common specifications, Version 1.40.* 2022.
- <span id="page-62-3"></span>[BSI-TR-03129-3] *BSI-TR-03129: Protocols for the Management of Certificates and CRLs in Public-Key-Infra structures (PKIs) Part 3: Electronic Identity (eID) documents based on Extended Access Control (EAC), Version 1.40.* 2022.
- <span id="page-62-20"></span>[BSI-TR-03129-3 Annex] *Annex to BSI TR-03129: Protocols for the Management of Certificates and CRLs in Pu blic-Key-Infrastructures (PKIs) Part 3: Common specifications, Version 1.40.* 2022.
- <span id="page-62-8"></span>[BSI-TR-03135-1] *BSI-TR-03135: Machine Authentication of MRTDs for Public Sector Applications Part 1: Over view and Functional Requirements.* 2021.
- <span id="page-62-14"></span>[BSI-TR-03137-1]*BSI-TR-03137: Optically Verifiable Cryptographic Protection of non-electronic Documents (Di gital Seal), Version 2.5.* 2021-12-03.
- <span id="page-62-16"></span>[EU DCC Imp. Decision] *COMMISSION IMPLEMENTING DECISION (EU) 2021/1073.* 2021-06-28.
- <span id="page-62-11"></span>[EU DCC JSON 1.3.0] *eHealth Network Guidelines on Technical Specifications for EU Digital COVID Certificates. JSON Schema Specification, Schema version: 1.3.0.* 2021-06-09.
- <span id="page-62-7"></span>[ICAO Doc 9303 p10] *ICAO Doc 9303, Machine Readable Travel Documents, Eighth Edition Part 10: Logical Data Structure (LDS) for Storage of Biometrics and Other Data in the Contactless Integrated Circuit (IC).* 2021.
- <span id="page-62-5"></span>[ICAO Doc 9303 p11] *ICAO Doc 9303, Machine Readable Travel Documents, Eighth Edition Part 11: Security Mechanisms for MRTDs.* 2021.
- <span id="page-62-9"></span>[ICAO Doc 9303 p13]*ICAO Doc 9303, Machine Readable Travel Documents, Eighth Edition Part 13: Visible Digital Seals.* 2021.
- <span id="page-62-18"></span>[ICAO Doc 9303 p3] *ICAO Doc 9303, Machine Readable Travel Documents, Eighth Edition Part 3: Specifications Common to all MRTDs.* 2021.
- <span id="page-62-12"></span>[ICAO Doc 9303 p7] *ICAO Doc 9303, Machine Readable Travel Documents, Eighth Edition Part 7: Rea dable Visas.* 2021.
- <span id="page-62-13"></span>[ICAO Doc 9303 p8] *ICAO Doc 9303, Machine Readable Travel Documents, Eighth Edition Part 8: Emergency Travel Documents.* 2021.
- <span id="page-62-15"></span>[ICAO TR DTAs] *Machine Readable Travel Documents Technical Report, Digital Travel Authorizations, v.2.15, ISO/IEC JTC1 SC17 WG3/TF1.* 2021.
- <span id="page-62-10"></span>[ICAO TR VDS-NC] *Visible Digital Seal for non-constrained environments, Version - 1.4.* 2022.
- <span id="page-62-1"></span>[KeepAChangelog] *Keep a Changelog – https://keepachangelog.com/en/1.0.0/.*
- <span id="page-62-4"></span>[RFC2119] *RFC 2119: Key words for use in RFCs to Indicate Requirement Levels.* 1997.
- <span id="page-62-17"></span>[RFC5280] *RFC 5280: Internet X.509 Public Key Infrastructure Certificate and Certificate Revocation List (CRL) Profile,.* 2008.

<span id="page-63-2"></span>[RFC5652] *RFC 5652: Cryptographic Message Syntax (CMS).* 2009.

<span id="page-63-1"></span>[RFC8392] *RFC 8392:CBOR Web Token (CWT).* 2018.

<span id="page-63-0"></span>[ČSN-36-9791] *Česká Technická Norma ČSN 36 9791: Information Technology – Country Verifying Certification Authority Key Management Protocol for SPOC. .* July 2009.