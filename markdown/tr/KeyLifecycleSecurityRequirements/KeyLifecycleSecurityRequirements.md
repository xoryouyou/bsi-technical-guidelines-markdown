...
|---|---|---|---|
|1.0|10.10.2016|BSI|First fnal version|
...
|1.0.3|17.11.2021|BSI|Defnition of options within Security Level 2 for<br>End Entities; minor editorial work.|
...
The requirements for Security Level 2 are a superset of the requirements for Security Level 1, i.e. fulfillment of the requirements for Security Level 2 implies compliance to Security Level 1. If a requirement is not marked to belong to one of the Security Levels, it applies to both levels. For End Entities, this document defines optional exceptions within Security Level 1 or 2 that MAY be applied. If applied, this MAY be denoted by Security Level 1_exc(<list of exceptions>) or Security Level 2_exc(<list of exceptions>), respectively, by listing the acronyms of all exceptions that apply and that are given in this document. 
...
The access to the Cryptographic Module MUST be protected by two factor authentication. Access to the Cryptographic Module MUST require at least two separately authenticated operators. For End Entities, the Root Certificate Policy MAY reduce this requirement to a single authenticated operator. If this exception is applied, the Root Certificate Policy MAY denote this by Security Level 2_exc(SAO). 
...
- For End Entities, the Root Certificate Policy MAY list additional application specific Protection Profiles. If this exception is applied, the Root Certificate Policy MAY denote this by Security Level 2_exc(AS_PP). 
...
- If one of the parts of [TR-03116] is applicable to the purpose the key is generated for, the requirements from that part MUST be fulfilled. Otherwise, the requirements from [TR-02102] MUST be fulfilled. The Root Certificate Policy MAY define derogations to this requirement. If this exception is applied, this MAY be denoted by Security Level 1_exc(KG) or by Security Level 2_exc(KG), depending on the Security Level that is otherwise satisfied by the entity. 
...
Importing a private key manually into a Cryptographic Module, e.g. when re-importing the backup key due to a hardware update, MUST require two independently authenticated operators. For End Entities, this requirement MAY be reduced by the Root Certificate Policy to a single authenticated operator for key import only. If this exception is applied, this MAY be denoted by Security Level 1_exc(SAO_KI) or by Security Level 2_exc(SAO_KI), depending on the Security Level that is otherwise satisfied by the End Entity. 
...
|---|---|
|EN419211-2|CEN: EN 419211-2: Protection profles for secure signature creation device -- Part 2:|
||Device with key generation|
|EN419211-3|CEN: EN 419211-3: Protection profles for secure signature creation device - Part 3:|
||Device with key import|
|EN419221-2|CEN: EN 419221-2 Protection profles for Trust Service Provider Cryptographic modules|
||-- Part 2: Cryptographic Module for CSP signing operations with backup - Protection|
||profle (CMCSOB-PP)|
|EN419221-4|CEN: EN 419221-4 Protection profles for Trust Service Provider Cryptographic modules<br>-- Part 4: Cryptographic module for CSP signing operations - Protection profle - CMCSO|
||PP|
|EN419221-5|CEN: EN 419221-5: Protection profles for Trust Service Provider Cryptographic modules|
...
|TR-02102|BSI: Technische Richtlinie TR-02102, Kryptografsche Verfahren: Empfehlungen und|
||Schlüssellängen|
