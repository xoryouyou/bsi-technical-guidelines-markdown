Klarstellungen und Anwendungshinweise zu BSI TR03153 und BSI-CC-PP-0105-V2-2020 
...
![](markdown/tr/TR-03153_Anwendungshinweise/TR-03153_Anwendungshinweise.pdf-0002-08.png)
...
Eine Technische Sicherheitseinrichtung enthält genau ein Sicherheitsmodul. Das Sicherheitsmodul jeder Technischen Sicherheitseinrichtung besteht aus einer Sicherheitsmodulanwendung und einem KryptoKern. Hierbei gilt: 
...
- Die SMAERS-Einheiten verschiedener Technischer Sicherheitseinrichtungen können technisch in einer oder mehreren sicherheitseinrichtungsübergreifenden SMAERS  zusammengefasst werden[1] . 

- Die CSP-Einheiten verschiedener Technischer Sicherheitseinrichtung können technisch in einem oder mehreren sicherheitseinrichtungsübergreifenden CSP zusammengefasst werden[2] . 
...
Die folgenden Abbildungen (Abbildung 2 und Abbildung 3) geben einen beispielhaften Überblick über mögliche Aufbauten des Sicherheitsmoduls. 
...
![](markdown/tr/TR-03153_Anwendungshinweise/TR-03153_Anwendungshinweise.pdf-0004-01.png)


## Abbildung 2 Mögliche Plattform-Architekturen 


![](markdown/tr/TR-03153_Anwendungshinweise/TR-03153_Anwendungshinweise.pdf-0004-03.png)
...
Daher MUSS beim zentralen Stellen der Zeit an einen zentralen CSP von der TSE unmittelbar ein SystemLog des Typs ‚updateTime‘ gemäß [TR-03151] erzeugt werden[3] . 

Die SE-API-Funktion „updateTime“ der einheitlichen Einbindungsschnittstelle MUSS auch unterstützt werden, wenn in der TSE keine Zeitstellung über die SE-API möglich ist. In diesem Fall MUSS von der SEAPI die Exception „ErrorFunctionNotSupported“ zurückgegeben werden. 
...
Sobald die Audit-Funktionalität der Sicherheitsmodulanwendung gestoppt wird, MUSS das Sicherheitsmodul permanent im Sinne der TR-03153 und TR-03151 deaktiviert werden und dies mit der „disableSecureElement“-System-Log-Nachricht der TR-03151 quittiert werden.[4] 
...
Dieser Abschnitt spezifiziert basierend auf BSI-TR-03153 und BSI TR-03151 die ergänzenden System-Logs. Log-Nachrichten DÜRFEN generell NICHT mit herstellerspezifischen Feldern ergänzt werden[5] . 
...
This message MUST be logged if an SMAERS-administrator authenticates towards the SMAERS unit [cf. PPSMAERS]. The systemOperationData octet string MUST consist of the following information. 
...
> 5 Die einzige Ausnahme hiervon ist die inhaltliche Ausgestaltung des Elementes seAuditData von AuditLog-Nachrichten durch den Hersteller. 
...
The message MUST be logged if an update or upgrade of the SMAERS component has completed[6] and SHOULD be logged if an update or upgrade of the other components of the TSE has completed. If a failure occurred, this message should be logged as well. The systemOperationData octet string MUST consist of the following information: 
...
The message MUST be logged if the SMAERS unit starts the audit function [cf. PP-SMAERS]. This functionality MUST NOT be available after initial configuration of the SMAERS unit. The systemOperationData octet string MUST NOT contain any data[7] : 
...
In case, the message cannot be logged at the time of event, the message MUST be logged immediately after all failures were resolved[8] . 
...
The message MUST be logged if all failures were resolved [cf. PP-SMAERS]. The systemOperationData octet string MUST NOT contain any data[9] . 

## 4.8 selfTest 

The message MUST be logged if any of the self tests of the secure element failed [cf. PP-SMAERS]. In case, the message cannot be logged at the time of event, the message MUST be logged immediately after all failures were resolved[10] . 
